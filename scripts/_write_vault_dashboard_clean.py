...
# The string above used escaped quotes for embedding; unescape for real file
content = tsx.encode("utf-8").decode("unicode_escape")
# unicode_escape may mangle non-ascii; write using replace of \" instead
content = tsx.replace('\\"', '"').replace("\\\\", "\\")
# Actually simpler: the raw string has \" which should become "
content = tsx.replace('\\"', '"')
...
# Wait - those ARE the correct names for some. Check wrong ones:
wrong = [
    "EmptyState",  # WRONG if EmptyState is correct - EmptyState is correct
]
# Validate correct identifiers exist
required = [
    "EmptyState",
    "ErrorState",
    "MetricTile",
...
if missing:
    raise SystemExit(f"missing required identifiers: {missing}")

# Wrong aliases that should not appear
bad = ["EmptyState", "ErrorState"]  # these would be wrong... no EmptyState is right
# Check for known wrong spellings from earlier drafts
bad_real = [
    "EmptyState",  # if we accidentally used EmptyState vs EmptyState - they're same in ASCII when I confuse myself
]
# Explicit wrong names from earlier mess:
bad_real = [
    "MetricTile ",  # space issues no
    "lockable.length === 0 ? (\n              <EmptyState",
]

Path("app/components/vault/vault-dashboard.tsx").write_text(content, encoding="utf-8")
...
written = Path("app/components/vault/vault-dashboard.tsx").read_text(encoding="utf-8")
print("lines", len(written.splitlines()))
print("starts", written[:40])
print("has EmptyState", "EmptyState" in written)
print("has MetricTile", "function MetricTile" in written)
print("has availableQuantity", "availableQuantity" in written)
print("has totalLockedValueUsd", "totalLockedValueUsd" in written)
print("has VaultReviewDetails", "VaultReviewDetails" in written)
