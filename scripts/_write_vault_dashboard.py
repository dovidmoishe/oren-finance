...
# Verify exact field names one more time in generated comments
portfolio = Path("app/types/portfolio.ts").read_text(encoding="utf-8")
vault = Path("app/types/vault.ts").read_text(encoding="utf-8")
states = Path("app/components/ui/states.tsx").read_text(encoding="utf-8")
print("availableQuantity" in portfolio, "availableQuantity" in portfolio)
print("EmptyState" in states, "EmptyState" in states)
print("totalLockedValueUsd" in vault, "programLive" in vault)
...
print("ok", Path("app/components/vault/vault-dashboard.tsx").stat().st_size)
